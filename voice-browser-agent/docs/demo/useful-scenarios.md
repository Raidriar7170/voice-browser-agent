...
| Public task-pack runner | selected useful task contracts | mixed read-only intents | Uses explicit task ids, deterministic or opt-in live mode, and honest non-success outcomes | local run manifest | local/private until sanitized |
...
The task-pack runner is the reproducibility layer for those useful contracts. It can produce deterministic non-network manifests for review and selected opt-in live manifests for local experiments. Both forms stay local/private and record site variance, missing proof, policy stops, and disabled configuration as evidence rather than successful public automation.
