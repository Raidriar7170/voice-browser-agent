...
6. Optional: enable an allowlisted docs target locally and run one `live_public_readonly` smoke command, emphasizing the task-contract, completion verifier, No login, read-only actions, private-by-default traces, and the bounded public-docs scope.
...
14. Note that GitHub public search remains a later public-readonly target; the current demo uses controlled showcase evidence unless a stable public task contract and sanitizer-approved evidence exist.
